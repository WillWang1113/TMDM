...
from utils.metrics import metric, prob_metric
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.use_multi_gpu and self.args.use_gpu:
            ...
            cond_pred_model = nn.DataParallel(
                cond_pred_model, device_ids=self.args.device_ids
            )
            cond_pred_model_train = nn.DataParallel(
                cond_pred_model_train, device_ids=self.args.device_ids
            )
    ...
    def _select_optimizer(self, mode="Model"):
        if mode == "Model":
            ...
            model_optim = optim.Adam(
                self.cond_pred_model_train.parameters(), lr=self.args.learning_rate_Cond
            )
        else:
            model_optim = None
        return model_optim
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        loss_vae = log_normal(
                            batch_y, y_0_hat_batch, torch.from_numpy(np.array(1))
                        )
                        ...
                        output = self.model(
                            batch_x, batch_x_mark, batch_y, y_t_batch, y_0_hat_batch, t
                        )

                        loss = (
                            e[:, -self.args.pred_len :, :]
                            - output[:, -self.args.pred_len :, :]
    ...
    def test(self, setting, test=0):
        ...
        def store_gen_y_at_step_t(config, config_diff, idx, y_tile_seq):
            ...
            gen_y = (
                ...
                .reshape(
                    ...
                    int(
                        config_diff.testing.n_z_samples
                        / config_diff.testing.n_z_samples_depart
                    ),
        ...
        def compute_true_coverage_by_gen_QI(
            config, dataset_object, all_true_y, all_generated_y
        ):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        y_0_hat_tile = (
                            y_0_hat_tile.transpose(0, 1).flatten(0, 1).to(self.device)
                        )
                        ...
                        x_mark_tile = (
                            x_mark_tile.transpose(0, 1).flatten(0, 1).to(self.device)
                        )
        ...
        mse, crps = prob_metric(preds, trues)
        ...
        np.save(os.path.join(folder_path, "metrics.npy"), np.array([mse, crps]))
        ...
        #     config=self.model.diffusion_config, y_true=y_true, all_gen_y=preds
        # )
        ...
        #     -1, preds_save.shape[-3], preds_save.shape[-2], preds_save.shape[-1]
        # )
